...
namespace ReadCalibox
{
    public partial class UC_Betrieb : UserControl
    {
        ...
        ****************************************************************************************************/
        ...
        private void NewLimits(int tag_nr)
        {
            ...
            if (H_TT.tSensor.tag_nr != tag_nr)
            {
                Limits.tag_nr = H_TT.tSensor.tag_nr;
                switch (tag_nr)
                {
                    case 6850:
                        Limits.sensor_id = 3989608;
                        Limits.item = "30646748";
                        var desc = "Converter O2";
                        //Limits.item = "52206119";
                        //var desc = "InPro6850i/12/120";
                        break;
                }
            }
        }
        ...
        private bool Check_Process(out int tagNo, out string message)
        {
            ...
            if (int.TryParse(H_TT.InputValue, out tagNo))
            {
                if (tagNo > 0)
                {
                    return true;
        ...
        private bool Check_TAGno_InUse(int tagNo)
        {
            foreach (UC_Channel channel in Config_ChannelsList)
            {
                if (channel.ItemValues.tag_nr == tagNo)
                {
                    ...
                    {
                        ErrorMessageMain = $"ERROR: TAG-Nr. {tagNo} Channel: {channel.ChannelNo}";
                        return false;
                    }
                    else { return true; }
                }
        ...
        ****************************************************************************************************/
        public void Channel_RunCheck(int tagNo, bool inUSE = false)
        {
            foreach (UC_Channel channel in Config_ChannelsList)
            {
                ...
                if (!channel.IsRunning)
                {
                    channel.IsStartReady = (tagNo > 0 && !inUSE);
                }
            }
        }
        public void Channel_started(int ch = -1)
        {
            if (ch > 0)
            {
                foreach (UC_Channel channel in Config_ChannelsList)
                {
                    if (channel.Active && !channel.IsRunning)
                    {
                        if (ch != channel.ChannelNo && channel.IsStartReady)
                        {
                            channel.IsStartReady = false;
                        }
                    }
                }
            }
            else
            {
                foreach (UC_Channel channel in Config_ChannelsList)
                {
                    if (channel.Active && !channel.IsRunning && channel.IsStartReady)
                    {
                        channel.IsStartReady = false;
                    }
                }
