        ...
        * Names:
                ...
                using (OdbcConnection con = ConnectionEK.NewConnection())
                {
                    using (OdbcCommand cmd = new OdbcCommand())
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            //ErrorHandler("Set_DB", exception: ex, message: $"ODBC: {odbc}\r\n{sqlQuery}");
                        }
                        finally
                        {
        ...
        public static void Set_Log(string odbc, DeviceResponseValues drv, ChannelValues limits)
        {
            if (drv.OpCode == OpCode.g901)
            {
                ...
                if (drv.Meas_I.IsMeasValue)
                {
                    DB_InserValue(ref sbHeader, ref sbValues, MN_I_set, drv.Iset.ValueNumeric, true);
                    DB_InserValue(ref sbHeader, ref sbValues, MN_I_AVG, drv.Iavg.ValueNumeric, true);
                    DB_InserValue(ref sbHeader, ref sbValues, MN_I_StdDev, drv.IstdDev.ValueNumeric, true);
                    DB_InserValue(ref sbHeader, ref sbValues, MN_I_Error, drv.Ierror.ValueNumeric, true);
                }
                if (drv.Meas_Temp.IsMeasValue)
                {
                    DB_InserValue(ref sbHeader, ref sbValues, MN_Temp_set, drv.TempSet.ValueNumeric, true);
                    DB_InserValue(ref sbHeader, ref sbValues, MN_Temp_AVG, drv.TempAvg.ValueNumeric, true);
                    DB_InserValue(ref sbHeader, ref sbValues, MN_Temp_StdDev, drv.TempStdDev.ValueNumeric, true);
                    DB_InserValue(ref sbHeader, ref sbValues, MN_Temp_Error, drv.TempError.ValueNumeric, true);
                }
                DB_InserValue(ref sbHeader, ref sbValues, MN_BoxErrorCode_HEX, drv.BoxErrorCode_hex, true);
                bool test_ok = false;
                switch (drv.BoxErrorCode_hex)
                {
                    case "0":
                    case "00":
                    case "2":
