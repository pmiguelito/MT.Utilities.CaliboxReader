namespace CaliboxLibrary
{
    public class DeviceLimitsResults
    {
        ...
        public void AddValues(DeviceResponseValues drv, DeviceLimits deviceLimits)
        {
            MeasValues = new DeviceMeasValues(drv);
            Limits = deviceLimits.Get_LimitsMode(drv.BoxMode_hex);
            Rating = new DeviceLimitsValuesRating() { Limits = Limits, MeasValues = MeasValues };
        }
    }
}
